...
let selectedGame = games[localStorage.getItem("selectedGame")]
...
function renderMain() {
  ...
  for (const game in games) {
    ...
    segmentsString +=
      '<div class="ui bottom attached segment tab" data-tab="' +
      games[game].id +
      '">' +
...
function renderLocations(game, darkTheme) {
  ...
  game.locations.forEach((location, index) => {
    ...
    const locationName = localStorage.getItem(
      game.id + location.value + "-name"
    );
    const locationEncounter = localStorage.getItem(
      game.id + location.value + "-encounter"
    );
    const locationNickname = localStorage.getItem(
      game.id + location.value + "-nickname"
    );
    ...
    string +=
      ...
      "</td>" +
      '<td data-sort-value="' +
      (locationEncounter ? escapeHTML(locationEncounter) : "") +
      '">' +
      '<div data-name="' +
      ...
      '-encounter" class="ui' +
      (darkTheme ? " inverted" : "") +
      ' fluid search selection long dropdown encounter-picker" aria-label="' +
      location.name +
      ' encounter">' +
      '<input value="' +
      (locationEncounter ? escapeHTML(locationEncounter) : "") +
      '" aria-label="' +
      location.name +
      ' encounter" name="pokemon" type="hidden">' +
      '<i class="dropdown icon"></i>' +
      ...
      "</td>" +
      '<td data-sort-value="' +
      (locationNickname ? escapeHTML(locationNickname) : "") +
      '">' +
      '<div class="ui' +
      (darkTheme ? " inverted" : "") +
      ' fluid input">' +
      ...
      game.nameLimit +
      '" class="nickname-input" value="' +
      (locationNickname ? escapeHTML(locationNickname) : "") +
      '" id="' +
      game.id +
      locationValue +
      '-nickname" name="nickname" placeholder="Nickname" type="text" aria-label="' +
      location.name +
      ' nickname">' +
      "</div>" +
      "</td>" +
      '<td data-sort-value="' +
      (locationStatus ? escapeHTML(locationStatus) : "") +
      '">' +
      '<div id="' +
      game.id +
      locationValue +
      '-status" class="ui' +
      (darkTheme ? " inverted" : "") +
      ' fluid selection long dropdown" aria-label="' +
      location.name +
      ' status">' +
      '<input value="' +
      (locationStatus ? escapeHTML(locationStatus) : "") +
      ...
      "</div>" +
      "</td>" +
      '<td><div title="Delete" class="ui' +
      (darkTheme ? " inverted" : "") +
      ' basic singleReset fluid icon button" data-location-id="' +
      locationValue +
      '"><i class="remove icon"></i></div></td>' +
      "</tr>";
  });
...
function resetGame(game, removeLocations) {
  ...
  if (removeLocations) {
    ...
    games[selectedGame].locations = games[selectedGame].locations.filter(
      (location) => location.value[0] !== "c"
    );
  }
}
    ...
    fr.onload = () => {
      try {
        ...
        if (data && data.hasOwnProperty("locations")) {
          ...
          $("#disableDexLimit").prop(
            "checked",
            data.settings && data.settings.disableDexLimit
          );
          ...
          if (data.customLocations.length) {
            const customLocations = filterByProperty(
              data.customLocations,
              "value"
            );

            localStorage.setItem(
              data.id + "-custom-locations",
        ...
        $("#messageContent").text(
          "The uploaded file was not recognized as valid JSON"
        );
      }
    };
...
function populateLocation(game, data) {
  ...
  if (data.encounter) {
    ...
    encounterElm.dropdown(
      "set text",
      '<i class="pkmn ' + data.encounter + '"></i>' + data.name
    );
...
function sortLocations(game) {
  const locations = games[game].locations.filter(
    (location) => location.value[0] !== "c"
  );

  let customLocations = JSON.parse(
    localStorage.getItem(game + "-custom-locations") || "[]"
  );
  ...
  if (initialLength) {
    ...
    while (customLocations.length !== 0) {
      ...
      customLocations.forEach((customLocation, index) => {
        const insertIndex = locations.findIndex(
          (e) => e.value == customLocation.order
        );
      ...
      customLocations = customLocations.filter(
        (location) => !locationRemoval.includes(location.value)
      );
...
function initTab(tab) {
  ...
  $("#" + tab + "-locations .encounter-picker").dropdown({
    ...
    onShow: function () {
      ...
      $(this).dropdown(
        "change values",
        localStorage.getItem(tab + "-disable-dex-limit")
          ? pkmnData
          : pkmnData
              .slice(0, games[tab].dexLimit)
              .filter(
                (pokemon) =>
                  pokemon.exclude === undefined ||
                  !pokemon.exclude.includes(tab)
              )
      );
  ...
  $("#" + tab + '-locations .encounter-picker[data-name!=""]').each(
    function () {
      const value = escapeHTML($(this).data("value"));
      $(this).dropdown("set value", value);
      $(this).dropdown(
        "set text",
        '<i class="pkmn ' + value + '"></i>' + $(this).data("name")
      );
    }
  );
...
function saveData(game) {
  ...
  games[game].locations.forEach((location) => {
    const encounter = localStorage.getItem(
      game + location.value + "-encounter"
    );
  ...
  link.setAttribute(
    "download",
    game + "." + new Date().toISOString().slice(0, 10) + ".json"
  );
  link.click();
}
...
function addLocation(location, game) {
  ...
  if (customLocations.length) {
    location.value =
      "c" +
      (parseInt(customLocations[customLocations.length - 1].value.slice(1)) +
        1);
  } else {
    location.value = "c0";
  }

  const duplicateLocationOrder = customLocations.findIndex(
    (e) => e.order == location.order
  );
...
function removeLocation(value, game) {
  ...
  const dependantLocation = customLocations.findIndex(
    (e) => e.order == location.value
  );
  ...
  customLocations.splice(
    customLocations.findIndex((e) => e.value == location.value),
    1
  );
...
function updateTab(game, updateDropdown) {
  ...
  $("#" + game + "-locations").html(
    renderLocations(games[game], localStorage.getItem("darkTheme") === "true")
  );
  initTab(game);
...
$(() => {
  ...
  $("#locationModal").modal({
    onApprove: () => {
      ...
      $("#customLocationName")
        .parent()
        .toggleClass("error", locationName == false);
  ...
  $("#" + selectedGame + "-locations").html(
    renderLocations(games[selectedGame], darkTheme)
  );
  ...
  $("#gameMenu .menu .item").tab({
    ...
    onLoad: (tabPath) => {
      ...
      $("#disableDexLimit").prop(
        "checked",
        localStorage.getItem(tabPath + "-disable-dex-limit")
      );
  ...
  $("#disableDexLimit").prop(
    "checked",
    localStorage.getItem(selectedGame + "-disable-dex-limit")
  );
